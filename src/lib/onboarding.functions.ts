  ...
  .handler(async ({ context }) => {
    ...
    if (data?.segment_id) {
      const { data: s } = await supabaseAdmin
        .from("ai_segments")
        .select("id,name,icon,segment_prompt,default_required_fields")
        .eq("id", data.segment_id)
        .maybeSingle();
      segment = s ?? null;
    }
  ...
  .inputValidator((input) =>
    z
      .object({
        ...
        ai_required_fields: z.array(z.string().max(64)).max(40).optional().nullable(),
        ...
        ai_can_send_photos: z.boolean().optional(),
      })
      .parse(input),
  )
  .handler(async ({ data, context }) => {
...
export const getWorkspaceProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    let segmentGeneralInfoDefaults: string[] = [];
    if (data?.segment_id) {
      const { data: seg } = await supabaseAdmin
        .from("ai_segments")
        .select("default_general_info_fields")
        .eq("id", data.segment_id)
        .maybeSingle();
      if (Array.isArray((seg as any)?.default_general_info_fields)) {
        segmentGeneralInfoDefaults = (seg as any).default_general_info_fields as string[];
      }
    }
    return {
      business_name: data?.business_name ?? "",
      ...
      business_general_info_custom: Array.isArray((data as any)?.business_general_info_custom)
        ...
        : [],
      business_general_info_notes:
        (data as any)?.business_general_info_notes &&
        typeof (data as any).business_general_info_notes === "object"
          ? ((data as any).business_general_info_notes as Record<string, string>)
          : {},
...
export const updateWorkspaceProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.business_general_info_notes !== undefined) {
      update.business_general_info_notes = data.business_general_info_notes;
    }
