...
export async function runAiResponse(input: AiRunInput): Promise<AiRunResult> {
  ...
  const bookingLayer = [
    ...
    '    - ANTES de resumir: se a última mensagem do cliente foi uma PERGUNTA (ex.: "amanhã às 14 tem disponível?", "o Bruno atende sábado?"), RESPONDA a pergunta primeiro, em linguagem natural ("Sim, amanhã às 14:00 está livre!" / "Nesse horário ele já está ocupado, mas às 15:00 tem vaga"). Só depois de responder é que você resume e pede a confirmação. Emendar direto no resumo sem responder soa robótico e ignora o que a pessoa perguntou.',
    '    - Resuma a ação proposta com estas informações: serviço, profissional, data, hora e nome de quem será atendido. Ex.: "Confirmo então: {serviço} com {profissional} em {data} às {hora}, no nome de {nome}. Posso confirmar?" — este é um MODELO do que precisa estar dito, não uma frase para copiar literalmente. Varie a redação para soar natural e encaixar no que já foi conversado.',
